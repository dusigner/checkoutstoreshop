...
export default class CustomProfileData {
  ...
  async addWhatsappOptIn() {
    ...
    if (!isShop) {
      return $('.optins-wrapper').prepend($field)
    }
  ...
  `);
    ...
    $('.optins-wrapper').prepend($container)
  }
  ...
  ensureOptinsWrapperExists() {
    if (!$('.optins-wrapper').length) {
      $('#client-profile-data p.save-data').after('<div class="optins-wrapper"></div>')
    }
  }
