...
export class CheckoutCustom {
  ...
  `;
		Object.assign(toast.style, {
			...
			padding: "12px 16px",
			borderLeft:
				type === "warning" ? "4px solid #f71963" : "4px solid #368df7",
			borderRadius: "4px",
			zIndex: 9999,
	...
	couponInfo(response) {
		...
		const couponExists = ratesAndBenefitsData.rateAndBenefitsIdentifiers.some(
			(item) => {
				return (
					item.matchedParameters &&
					item.matchedParameters["couponCode@Marketing"] ===
		...
		try {
			const couponInfoElement = $(
				'<div class="div-coupon-info"><p style="font-size: 12px; color: #000;"></p></div>',
			);
			...
			if (marketingData && marketingData.coupon) {
				if (couponExists) {
					...
					couponInfoElement
						.find("p")
						.text("Cupom de desconto aplicado")
						.css("color", "#006BEA");
				...
				couponInfoElement
					.find("p")
					.text("Cupom inválido para compra")
					.css("color", "red");
				...
				if (messages && messages.length > 0) {
					...
					if (
						["giftCardCommunicationError", "invalidGiftCard"].includes(
							errorMessage?.code,
						)
					) {
					...
					if (warningMessage) {
						...
						if (text === "O valor dos itens foi alterado") {
							...
							couponInfoElement
								.find("p")
								.text("Digite o cupom de desconto")
								.css("color", "#000");
							couponFields.append(couponInfoElement);
							return;
						}
						...
						if (!isReward && isRewardsCoupon) {
							couponInfoElement
								.find("p")
								.text(
									"Esse cupom é para uso exclusivo do Portal Rewards! Acesse agora para finalizar sua compra",
								)
								.css("color", "red");
	...
	showCustomMsgInstallation(orderForm) {
		try {
			...
			const installationSummaryRows = installationServices.map((item) => {
				...
				const textColor =
					installationPrice !== "Grátis" ? "color: #000 ;" : "color: #2189FF";
	...
	showCustomMsgCoupon(orderForm) {
		...
		const couponItemsCount = orderForm.items.reduce(
			(accumulator, item) =>
				accumulator +
	...
	summaryCustom() {
		try {
			...
			const quantitySelectedItems = quantitySelectedItemsArray.reduce(
        ...
        <div class="summaryOrder">
          <h6>Resumo do pedido (${quantitySelectedItems} ${
						quantitySelectedItems <= 1 ? "item" : "itens"
					})</h6>
          <ul>
            ${listItems}
          </ul>
	...
	enchancementTotalPrice(orderForm) {
		...
		try {
          ...
          <div class="v-custom-quantity-price vqc-ldelem">
            ...
            font-weight:bold; ${free && !samsungCare?.length ? "color: #2189FF;" : ""}" >${
	...
	subTotalSummary(orderForm) {
		try {
			...
			const _elementListPrice =
				$("td.product-price").find(".new-product-price");

			const valoresSubTotalArray = [];
			...
			const discountFinalFormatted = formatNegativeValue(
			...
			if (orderForm.value === _this.subTotalValueFinal) {
				...
				if (_this.discountPrices) {
					const hasDiscount = orderForm.totalizers?.filter(
						(val) => val.id === "Discounts",
					);
					const discountTotal = _this.discountPrices + hasDiscount[0]?.value;
					_discountElement.text(
						formatNegativeValue(formatCurrencyBRL(discountTotal)),
					);
					if (hasDiscount.length > 0) {
						if (
							_containerTotalizers.find(".discount-subtotal-container")
								.length === 0
						) {
	...
	applyStepVisibilityConditions() {
		if (!this.isMobile) return;

		const hash = window.location.hash;

		const $clientProfile = $("#client-profile-data");
		const $shippingData = $("#shipping-data");
		const $miniCart = $(".cart-template.mini-cart.span4");
			...
			if (pixPayment) {
				setTimeout(() => {
					...
					const interval = setInterval(() => {
						if (
							$(
								".body-order-form .container-order-form .transactions-container .VTEX-PIX__modal-background",
							).length
						) {
	...
	showMessagePix(orderForm) {
		try {
			...
			const _containerQRCode = $(
				".body-order-form .container-order-form .transactions-container .VTEX-PIX__modal-background .VTEX-PIX__qrcode-container",
			);
			const _containerPixHeaderDescription = $(
				`.body-order-form .container-order-form .transactions-container .VTEX-PIX__modal-background ${this.isMobile ? ".VTEX-PIX__center-container-head-mobile" : ".VTEX-PIX__container-info-head"}`,
			);
			const _containerPixDescription = $(
				`.body-order-form .container-order-form .transactions-container .VTEX-PIX__modal-background ${this.isMobile ? ".VTEX-PIX__center-container-subhead-mobile" : ".VTEX-PIX__container-info-description"}`,
			);
			const _containerPixDescriptionInfos = $(
				".body-order-form .container-order-form .transactions-container .VTEX-PIX__modal-background .VTEX-PIX__container-info",
			);
			const _containerPixValueFooter = $(
				".body-order-form .container-order-form .transactions-container .VTEX-PIX__modal-background .VTEX-PIX__footer .VTEX-PIX_footer-value",
			);
			const _containerPixElementValue = _containerPixValueFooter.find(
				".VTEX-PIX_footer-value-samsung",
			);
			const _containerPixElement =
				_containerPixHeaderDescription.find(
					".VTEX-PIX__container-info-head-samsung",
				) &&
				_containerPixDescription.find(
					".VTEX-PIX__container-info-head-samsung",
				) &&
				_containerPixDescriptionInfos.find(
					".VTEX-PIX_description-footer-samsung",
				);
	...
	showMessageMercadoPagoPayment(orderForm) {
		try {
			if (orderForm.items === 0) return;
			const _containerTotalizers = $(
				".box-step .box-step-content .steps-view .box-payment-mercadopago .mercadopago-box-wrapper",
			);
			const _containerMercadoPagoElement = _containerTotalizers.find(
				".mercadopago-box-wrapper-samsung",
			);
	...
	showMessageNubankPayment(orderForm) {
		try {
			if (orderForm.items === 0) return;
			const _containerTotalizersNubankExtra = $(
				".box-step .box-step-content .steps-view .box-payment-nubank .nu-container .nu-extra-limit",
			);
			const _containerNubankExtraElement = _containerTotalizersNubankExtra.find(
				".nu-extra-limit-samsung",
			);
        ...
        `;
			...
			const _containerTotalizers = $(
				".box-step .box-step-content .steps-view .box-payment-nubank .nu-container .nu-body",
			);
			const _containerNubankElement = _containerTotalizers.find(
				".nu-body-items-samsung",
			);
	...
	async enchancementSummaryCart(orderForm, path) {
		try {
          ...
          <div class="cart-total" style="margin-bottom: 20px; color: #000">
            <div class="best-price" style="font-size: 20px; display: flex; justify-content: space-between; font-weight: 700">
        ...
        `;
			...
			if (
				...
				const paymentSystem =
					accountPaymentMap[account] || accountPaymentMap.default;
				...
				const creditCardPaymentGroup =
					orderForm?.paymentData?.paymentSystems?.find(
						(paymentSystem) =>
				...
				if (_this.lastOrderFormTotalPrice !== orderForm.value) {
					...
					if (creditCardPaymentSystemId) {
						const creditCardInstallments =
							await getMaxInstallmentByPaymentSystem(creditCardPaymentSystemId);
          ...
          ${
                ...
                <p style="margin-bottom: 0;">
                  ...
                  ${!_this.maxInstallmentSamsungCard?.total ? `<span class="custom-tooltip">i</span>` : ""}
                </p>
	...
	itauCardMessage(orderForm) {
		if (orderForm && $(".itauCardMessage").length === 0) {
			if (orderForm.paymentData) {
				const itauCardMessagMastereHtml = `<div class="itauCardMessage">
          ...
          <p class="itauCardMessage__mastercardFlag">Bandeira Mastercard: até <b>21x</b> sem juros</p>
				...
				const itauCardMessagVisaeHtml = `<div class="itauCardMessage">
          <h2 class="itauCardMessage__title">Importante</h2>
          <p class="itauCardMessage__visaFlag">Bandeira Visa: até <b>24x</b> sem juros</p>
        </div>`;

				const itauCardSelectMasterCardElement = $(
					".steps-view .pg-samsung-itaucard-master",
				);
				const itauCardSelectVisaElement = $(
					".steps-view .pg-samsung-itaucard-visa",
				);
	...
	verifyCSP(orderForm) {
		try {
			...
			const statePickUp = filterSlas[0].pickupStoreInfo.address?.state;
	...
	defaultGiftCard(orderForm, defaultId = "VtexGiftCard") {
		...
		try {
			...
			if (giftCardsVtex.length === 0) {
				setTimeout(() => {
					$("body").on("click", "#show-gift-card-group", () => {
						setTimeout(() => {
			...
			console.error(
				`Erro ao definir o tipo de voucher padrão selecionado: ${err}`,
			);
		}
	}
	...
	bind() {
		...
		$("body").on("click", ".vtex-omnishipping-1-x-linkEdit.link-edit", () => {
			setTimeout(() => {
				if (_this.customAddressForm) {
					$("body").addClass("v-custom-addressForm-on");
				}
		...
		$("body").on("click", "#btn-client-pre-email", () => {
			setTimeout(() => {
		...
		$("body").on("click", "#shipping-option-delivery", () => {
			_this.customAddressFormInit(_this.orderForm);
		});

		$("body").on("click", ".VTEX-PIX__dry-button", () => {
			setTimeout(() => {
				_this.showMessagePix(_this.orderForm);
			}, 10);
		});

		$("body").on("click", ".show-more-items-button", () => {
			_this.general();
		});
	...
	start() {
		...
		try {
			...
			$(window).on("checkoutRequestBegin.vtex", (event, request) => {
			...
			$(window).on("attachmentUpdated.vtex", (evt, orderFormSection) => {
			...
			$(window).load(async () => {
				...
				if (
					(window.vtex.accountName === "samsungbrshop" ||
						window.vtex.accountName === "samsungbrtests") &&
					!window.location.host.includes("storeplus")
				) {
					adobeLaunchInit();
				}
	...
	changeToAnonymousUserAndReload(orderFormId) {
		fetch(`${rootPath()}/checkout/changeToAnonymousUser/${orderFormId}`)
			.then(() => {
